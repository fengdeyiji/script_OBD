...
parser.add_argument('--src_dir', type=str, required=True,
                    help='OB源代码绝对路径，必填')
parser.add_argument('--deploy_dir', type=str, required=True,
                    help='部署目录名称，绝对路径，若目录不存在则新建，必填')
parser.add_argument('--deploy_ip_list', type=str, nargs='+', required=False,
                    help='要部署的IP列表，默认为本机两副本部署，如果在同一IP上部署多个Observer需要把IP重写多次')
...
parser.add_argument('--tag', type=str, default='test', required=False,
                    help='生成的OBD镜像名称以及部署的集群名称，默认为test')
parser.add_argument('--with_admin',  default=False, type=bool,
                    help='编译并复制ob_admin，默认不编译')
args = parser.parse_args()

server_config = ''

self_ip = os.popen('hostname -i').read().strip()
print('self_ip:{}'.format(self_ip))
if args.deploy_ip_list is None:
  args.deploy_ip_list = [self_ip, self_ip]# 如果不填写部署ip，那么就在本机两副本部署
...
config_yaml = '''oceanbase-ce:
  ...
  global:
    ...
    cpu_count: 4 # The assigned cpu for an observer
    ...
    __min_full_resource_pool_memory: 1073741824 # Lower bound of memory for resource pool, default 5G{}'''.format(args.tag, server_config, devname, server_config_detail)
